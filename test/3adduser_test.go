...
func TestAddUser(t *testing.T) {
	...
	router.POST("/authorize/checkadmin/adduser", authorize.ExtractJwtMiddleware(), controller.CheckIfAdminUserMiddleware(), controller.AddUser)

	requestData := []byte(`{"username": "u5name","password": "u5pword"}`)

	// Create a new HTTP request
	req, err := http.NewRequest("POST", "/authorize/checkadmin/adduser", bytes.NewBuffer(requestData))
	...
	jsonMessage := "new user succesfully added"
